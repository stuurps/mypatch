...
| B12 | Empty state quality | S | What a brand new user sees before their first log. The current state ("Nothing logged yet") is functional but cold. After a warm four-screen onboarding the landing should feel inviting, not blank. Also covers poster and journal empty states. |
| B13 | First sighting onboarding prompt | S | A quiet nudge immediately after onboarding — "Your patch is set. What's out there today?" Visible on home until first log, then gone permanently. One-time, non-repeating, not a modal. Bridges the onboarding-to-first-log gap. Store dismissed state in settings table. |
